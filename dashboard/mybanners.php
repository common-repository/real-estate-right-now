...
if ($cached_coupon_data !== '' && $cached_coupon_data !== false) {
    ...
    echo '<img src="' . esc_url(REALESTATERIGHTNOWIMAGES) . '/' . esc_attr($image) . '" style="width: 100%; height: auto;" />';
    echo "<br>";
    ...
    if ($title && $message_text) {
        echo '<ul>';
        echo '<h2>' . esc_html($title) . '</h2>';
        echo '<img src="' . esc_url(REALESTATERIGHTNOWIMAGES) . '/news.gif" style="width: 100%; height: auto;" />';
        echo "<br>";
...
$x = rand(1, 2);
if ($x < 2) {
    echo '<h2>' . esc_attr__("Did you like the Real Estate Right Now Plugin?", "real-estate-right-now") . '</h2>';
    echo '<img src="' . esc_url(REALESTATERIGHTNOWIMAGES) . '/help3.jpg' . '" style="width: 100%; height: auto;" />';
    esc_html_e('Please support us by rating our plugin on WordPress.org. Help us keep this plugin live and updated.', 'real-estate-right-now');
?>
    <br /><br />
    <a href="https://wordpress.org/support/plugin/real-estate-right-now/reviews/#new-post" class="button button-medium button-primary"><?php _e('Rate', 'real-estate-right-now'); ?></a>
<?php
} else {
    echo '<h2>' . esc_attr__("Please help us keep the plugin live & up-to-date", "real-estate-right-now") . '</h2>';
    echo '<img src="' . esc_url(REALESTATERIGHTNOWIMAGES) . '/help1.jpg' . '" style="width: 100%; height: auto;" />';

    esc_attr_e('If you use & enjoy Real Estate Right Now Plugin, please rate it on WordPress.org. It only takes a second and helps us keep the plugin live and maintained. Thank you!', 'real-estate-right-now');
?>
    <br />
    <a href="https://wordpress.org/support/plugin/real-estate-right-now/reviews/#new-post" class="button button-medium button-primary"><?php esc_attr_e('Rate', 'real-estate-right-now'); ?></a>
<?php
}
