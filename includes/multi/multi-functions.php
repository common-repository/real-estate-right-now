...
function realestaterightnow_fields_columns_head($defaults) {
...
function realestaterightnow_fields_columns_content($column_name, $post_ID) {
...
add_filter( 'manage_edit-realestatefields_sortable_columns', 'realestaterightnow_fields_sortable_column' );

function realestaterightnow_fields_sortable_column( $columns ) {
...
function realestaterightnow_multifields_list($query) {
...
if(isset($_GET['post_type'])){
    ...
    if (sanitize_text_field($_GET['post_type']) == 'realestatefields')

      {

        // add_action('pre_get_posts', 'realestaterightnow_multifields_list');

        add_filter('manage_realestatefields_posts_columns', 'realestaterightnow_fields_columns_head');

        add_action('manage_realestatefields_posts_custom_column', 'realestaterightnow_fields_columns_content', 10, 2);
